...
Our cohort consisted of 14 specimens from 6 patients enrolled in a prospective clinical trial (NCT05914792)^14,15^ evaluating primary endocrine therapy (aromatase inhibitors) for women aged 70 and older with ER+/HER2- breast cancer who chose to forego upfront surgery. The study collected matched core biopsy and surgical specimens from each patient; some samples include technical replicates, yielding the full set of 14 specimens. Two patients were classified as responders based on imaging assessment and circulating tumor DNA (ctDNA) monitoring; these patients elected surgery around the 18-month mark for reasons unrelated to treatment failure, yielding 5 specimens. Four patients were classified as progressors, showing clinical or ctDNA-based evidence of disease progression around 36 months, at which point surgery was required, yielding 9 specimens. To preserve patient anonymity, all samples are labeled HCC22-088-P#-S#^15^.

Module 4 applied non-negative matrix factorization to the deconvolved cell type-specific expression layers from Module 3, discovering gene expression programs within each cell type population (Figure 5A). We validated spatial coherence using Moran's I: programs with significant positive spatial autocorrelation (I > 0.2, p < 0.01) exhibit spatial clustering, indicating biologically coordinated activity rather than technical noise. Across the cohort, 68% of discovered programs exceeded the threshold for moderate spatial coherence. Fibroblast programs showed the highest coherence (mean I = 0.28), consistent with the organized spatial distribution of stromal cells, while T cell programs showed more variable coherence (I range 0.08-0.26), reflecting heterogeneity in immune infiltration patterns. Bivariate Moran's I between program pairs identified co-localized programs representing coordinated multicellular activities and mutually exclusive programs representing competing cellular states.

Module 5 integrated programs across all 14 specimens, aligning them in a shared latent space using PCA followed by Harmony batch correction to remove patient-specific technical variation while preserving biological signal. Hierarchical clustering identified 73 aligned programs representing conserved transcriptional states from 590 individual programs across the cohort (Figure 5B). Of these, 5 programs appeared in more than half the specimens, indicating highly conserved biology, while 36 were specimen-specific.

Response analysis compared program enrichment between responder and progressor specimens (Figure 5C). Three programs were significantly enriched in responders, including a macrophage program characterized by FABP4, HBA2, and TNXB, and a cancer luminal program marked by MGP, MT-CO3, and FOS. Four programs were enriched in progressors, including a dendritic cell program with FTL, FN1, and TIMP1, and a cancer basal program with mitochondrial gene signatures. Differential expression analysis using PyDESeq2^9^ on pseudo-bulk aggregated expression identified 127 significantly differentially expressed genes (adjusted p < 0.05) between the 5 responder and 9 progressor specimens across 13,371 tested genes (Figure 5D). The vast majority (122 genes) were upregulated in progressors, including matrix metalloproteinases (MMP13, MMP3, ADAMTS4, ADAMTS15), the necroptosis effector MLKL, and immune modulators (ALOX5AP, CLEC5A). Only 5 genes were upregulated in responders, including TMEM38B and TRIM72. Pathway analysis via GSEApy showed progressor-enriched genes were enriched for extracellular matrix organization (GO:0030198), collagen catabolism (GO:0030574), and the Hallmark epithelial-mesenchymal transition signature.

Module 5 also identified 191 conserved spatial relationships, of which 26 (13.6%) showed consistent co-localization across patients, 6 (3.1%) showed consistent mutual exclusion, and the remainder were independent. This relationship network highlights multicellular organization patterns that persist despite inter-individual variation.
...
Our clinical cohort is limited in size (n = 6 patients, with responders from only 2 patients), and response-associated findings should be interpreted as hypothesis-generating rather than definitive. The correlation structure of multiple specimens per patient was not accounted for in the differential expression analysis. Validation in larger, independent cohorts is required to confirm the biological relevance of the response-associated programs reported here.
...
**Differential Expression Analysis.** Pseudo-bulk aggregation sums gene expression within each sample. PyDESeq2 (v0.4)^9^ performs differential expression with design formula ~condition (responder vs progressor). Genes with adjusted p < 0.05 are considered significant.
...
Supplementary Figure S3: T47D ELISA and immunofluorescence results.
