...
# Set file paths
files <- list(
  "CITEgeist" = "CITEgeist_mixed_runtime.log",
  "RCTD" = "RCTD_mixed_runtime.log",
  "Seurat" = "Seurat_mixed_runtime.log",
  "Cell2Location" = "cell2location_mixed_runtime.log",
  "Tangram" = "tangram_mixed_runtime.log"
)
...
p <- ggplot(runtime_data, aes(x = Method, y = Runtime_Seconds, fill = Method)) +
  ...
  labs(
    ...
    subtitle = sprintf("Mixed; ANOVA: F = %.2f, p = %.3g", anova_Fstat, anova_pval),
    x = "Method",
    y = "Runtime (seconds)"
  ) + 
...
# Save plot
ggsave("mx_runtime_comparison.png", plot = p, width = 8, height = 5, dpi = 300)
