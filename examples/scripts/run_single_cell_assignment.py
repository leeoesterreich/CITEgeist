...
Usage:
  python run_morphology_assignment.py --sample sample-P1-S1 [--stages 1,2,5,6]
"""
...
def stage5_assign_and_gex(
    ...
    if emb_list:
        ...
        logger.info("  Assembled embeddings: %d/%d cells have embeddings", has_emb.sum(), len(cell_ids))
    else:
    ...
    cell_spot_map = cell_spot_map.rename(
        columns={
            "nucleus_id": "cell_id",
            "centroid_x": "x",
            "centroid_y": "y",
...
def stage6_visualize(
    ...
    for type_idx, ct_name in enumerate(CELL_TYPES):
        ...
        if len(xs) > 0:
            sc_plot = ax.scatter(
                xs, ys, c=ws, cmap="hot", s=2, alpha=0.7, vmin=0, vmax=np.percentile(ws, 95), linewidths=0
            )
...
def main():
    ...
    parser.add_argument("--stages", default="1,2,5,6", help="Comma-separated stages to run (default: 1,2,5,6)")
    ...
    parser.add_argument(
        "--pooled-checkpoint", type=str, default=None, help="Path to pooled MIL checkpoint (skips per-sample Stage 3)"
    )
    parser.add_argument(
        "--module3-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Read embeddings from this dir instead of output_dir " "(for reusing Stage 1-2 outputs)",
    )
    parser.add_argument(
        "--output-dir-override",
        type=str,
