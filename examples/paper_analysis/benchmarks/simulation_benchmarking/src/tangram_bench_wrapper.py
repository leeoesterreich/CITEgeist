...
def main(
    replicates_dir, ground_truth_gex_dir, ground_truth_proportions_dir, output_dir, folder_pattern, normalize="range"
):
    ...
    for replicate_folder in os.listdir(replicates_dir):
        ...
        replicate_number = replicate_folder.split("_")[-1]  # Extract the number from tangram_map_<i>
        ...
        # Predicted proportions file path (from Tangram output)
        predicted_proportions_file = os.path.join(replicate_path, "cell_type_proportions.csv")

        # Check if required files are present
        if (
            not os.path.isdir(predicted_gex_dir)
            or not os.path.exists(predicted_proportions_file)
            or not os.path.exists(ground_truth_proportions_file)
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "ground_truth_gex_dir", type=str, help="Path to the directory containing ground truth GEX files."
    )
    parser.add_argument(
        "ground_truth_proportions_dir", type=str, help="Path to the directory containing ground truth proportion files."
    )
    parser.add_argument("output_dir", type=str, help="Path to save the benchmarking results.")
    parser.add_argument(
        "--folder_pattern",
        type=str,
        default="tangram_map_",
        help="Pattern to match replicate folders (default: 'tangram_map_').",
    )
    parser.add_argument(
        "--normalize",
        type=str,
    ...
    main(
        args.replicates_dir,
        args.ground_truth_gex_dir,
        args.ground_truth_proportions_dir,
        args.output_dir,
        args.folder_pattern,
        args.normalize,
    )
