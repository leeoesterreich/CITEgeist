...
def parse_args():
    ...
    parser.add_argument(
        "--seurat_results_dir", type=str, required=True, help="Directory containing Seurat result files."
    )
    parser.add_argument("--ground_truth_dir", type=str, required=True, help="Directory containing ground truth files.")
    parser.add_argument(
        "--output_dir", type=str, required=True, help="Directory to save the output benchmarking metrics."
    )
...
def main():
    ...
    seurat_prediction_files = sorted(
        [
            os.path.join(seurat_results_dir, f)
            for f in os.listdir(seurat_results_dir)
            if f.endswith("Seurat_deconv_predictions.csv")
        ]
    )
