...
def load_xenium_data(
    ...
    if len(common_cells) < len(adata.obs_names):
        logger.warning(f"Some cells in matrix not in metadata: " f"{len(adata.obs_names) - len(common_cells)} missing")
    ...
    adata.obsm["spatial"] = np.column_stack([adata.obs["x_centroid"].values, adata.obs["y_centroid"].values])
...
def split_gex_protein(
    ...
    logger.info(f"Splitting: {gex_mask.sum()} genes, {protein_mask.sum()} proteins")
