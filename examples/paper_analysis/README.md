...
| Script | Produces |
|---|---|
| `mdk_analysis/scripts/generate_pergene_morans_heatmap.py` | per-gene bivariate Moran's I heatmap (MDK vs each secretory gene, per specimen) |
...
- **A small number of upstream producers.** Three inputs consumed by the shipped scripts are
  generated by code that is not part of this curated set:
  - the CITEgeist bivariate Moran's baseline (`bivariate_morans_results.csv`) that Figure 5C
    compares the competitors against;
  - the per-gene Moran's table that `generate_pergene_morans_heatmap.py` reads;
  - the biopsy→surgery trajectory deltas behind Figure 5E.

  The metric definitions for the first two are reproduced in
  `benchmarks/competitor_gex_validation/run_bivariate_morans.py`, which states explicitly
  that it computes the same quantity.
