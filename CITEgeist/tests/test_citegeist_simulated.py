...
def calculate_gex_metrics(ground_truth_dir, layer_dir, pass_number=None):
    ...
    for celltype, metric in metrics.items():
        if (
            metric["RMSE"] is None
            or metric["NRMSE"] is None
            or metric["MAE"] is None
            or np.isnan(metric["RMSE"])
            or np.isnan(metric["NRMSE"])
            or np.isnan(metric["MAE"])
        ):
    ...
    metrics_values = {
        ...
        "Metric": ["Average RMSE", "Median RMSE", "Average NRMSE", "Median NRMSE", "Average MAE", "Median MAE"],
        "Value": [
...
def calculate_improvements(pass1_metrics, pass2_metrics):
    ...
    return pd.DataFrame({"Metric": list(improvements.keys()), "Improvement_Percentage": list(improvements.values())})
...
def main():
    ...
    parser.add_argument("--lambda_reg", type=float, required=True, help="Regularization strength for elastic net")
    parser.add_argument("--alpha_elastic", type=float, required=True, help="Elastic net mixing parameter (0=L2, 1=L1)")
    parser.add_argument(
        "--max_y_change", type=float, required=True, help="Maximum allowed change in Y values between iterations (0,1)"
    )
    ...
    parser.add_argument(
        "--skip_pass2", action="store_true", default=False, help="If set, skip pass 2 and only run pass 1."
    )
    ...
    # Initialize logging
    log_file = (
        f"Simulated_CITEgeist_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{radius}_{lambda_reg}_{alpha_elastic}.log"
    )
    log_path = os.path.join(args.output_folder, log_file)
    logging.basicConfig(
        filename=log_path, filemode="w", format="%(asctime)s - %(levelname)s - %(message)s", level=logging.DEBUG
    )
    logging.info(
        f"Starting CITEgeist run with parameters: radius={radius}, lambda_reg={lambda_reg}, alpha_elastic={alpha_elastic}"
    )
    ...
    for number in sample_numbers:
        ...
        spot_composition_df = (
            pd.read_csv(os.path.join(st_folder, f"Wu_ST_{number}_prop.csv"), index_col=0)
            .sort_index()
            .sort_index(axis=1)
        )
        ...
        results_dict = {"global": global_cell_type_proportions_df, "finetune": finetuned_cell_type_proportions_df}
        ...
        for key, test_spots_df in results_dict.items():
            ...
            if not np.array_equal(test_spots_df.index, spot_composition_df.index):
                logging.warning(
                    f"test_spots_df indices: {test_spots_df.index}, spot_composition_df indices: {spot_composition_df.index}"
                )
                raise ValueError(
                    "ERROR: The row indices in the input CSV files do not match or are not in the same order!"
                )

            # Check if columns match
            if not np.array_equal(test_spots_df.columns, spot_composition_df.columns):
                logging.warning(
                    f"test_spots_df columns: {test_spots_df.columns}, spot_composition_df columns: {spot_composition_df.columns}"
                )
                raise ValueError(
                    "ERROR: The column names in the input CSV files do not match or are not in the same order!"
                )
            ...
            prop_results_name = os.path.join(
                output_folder, f"{sample_name}_cellprop_results_summary_{key}_{suffix}_{radius}_.csv"
            )
        ...
        pass1_results = model.run_cell_expression_pass1(
            radius=radius, max_workers=None, checkpoint_interval=100, output_dir="checkpoints", rerun=True
        )
