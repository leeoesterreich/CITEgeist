...
def load_and_preprocess():
    ...
    logger.info(f"Cancer cells: {adata_cancer.n_obs} " f"({dict(adata_cancer.obs['cell_type'].value_counts())})")
    ...
    logger.info(
        f"Gene filter: {n_genes_before} -> {adata_nmf.n_vars} genes "
    ...
    if krt5_cols:
        ...
        logger.info(f"Subtype by KRT5 gate: {krt5_pos.sum()} KRT5-high, " f"{(~krt5_pos).sum()} KRT5-low")
    else:
...
def run_nmf_stability(adata_nmf):
    ...
    X = np.asarray(adata_nmf.X.copy() if not hasattr(adata_nmf.X, "toarray") else adata_nmf.X.toarray())
    ...
    for k in NMF_K_VALUES:
        model = NMF(n_components=k, init="nndsvda", random_state=PRIMARY_SEED, max_iter=500)
        ...
        if "MDK" in gene_names:
            ...
            logger.info(
                f"  K={k}: recon_error={model.reconstruction_err_:.4f}, "
...
def compute_nmf_stability(seed_results, gene_names):
    ...
    logger.info(
        f"NMF stability: same_rank={all_same_rank}, "
...
def run_region_enrichment(H, adata_cancer):
    ...
    for k in range(n_programs):
        ...
        logger.info(
            f"  Program {k}: FC={fc:.2f} ({enriched_in}), "
...
def identify_mdk_program(W, gene_names, enrichment_df):
        ...
        mdk_loadings.append(
            {
                ...
                "basal_enriched": (
                    bool(enrichment_df.loc[enrichment_df["program"] == k, "significant"].values[0])
                    if len(enrichment_df) > 0
                    else False
                ),
                "top_5_genes": ", ".join([gene_names[i] for i in np.argsort(W[:, k])[::-1][:5]]),
            }
        )
    ...
    for _, row in mdk_df.iterrows():
        logger.info(
            f"  Program {row['program']}: MDK={row['mdk_loading']:.4f}, "
    ...
    logger.info(f"Context genes (top 100): {n_secretory} secretory, " f"{n_receptor} MDK receptors, {n_er} ER-related")
...
def write_outputs(
    W, H, gene_names, adata_cancer, enrichment_df, mdk_program, mdk_df, context_df, moran_df, stability, k_results
):
    """Write all CSV outputs and comparison report."""
    logger.info(f"Writing outputs to {OUTPUT_DIR}/")

    # Program loadings (genes x programs)
    loadings_df = pd.DataFrame(W, index=gene_names, columns=[f"program_{k}" for k in range(W.shape[1])])
    ...
    activities_df = pd.DataFrame(H, index=cell_ids, columns=[f"program_{k}" for k in range(H.shape[1])])
...
def write_comparison_report(W, gene_names, enrichment_df, mdk_program, context_df, moran_df, stability, adata_cancer):
    ...
    if mdk_program is not None and "MDK" in gene_names:
        ...
        lines.append(
            f"KRT5-high enrichment: FC={enrich_row['fold_change']:.2f}, "
