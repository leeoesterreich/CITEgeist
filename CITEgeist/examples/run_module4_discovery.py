...
Usage:
    python run_module4_discovery.py --sample sample-P1-S1 --output-dir output/module4_discovery

    # Run both anchored and joint discovery:
    python run_module4_discovery.py --sample sample-P1-S1 --mode both
"""
import argparse
import json
import logging
...
from CITEgeist.model.programs.anchored_program_discovery import discover_anchored_programs, discover_joint_programs
...
# Use 7-type cuOPT QP Module 3 outputs
...
# Sample metadata
SAMPLE_METADATA = {
    "sample-P1-S1": {"patient": "P1", "timepoint": "S1", "response": "Progressor"},
    "sample-P1-S2": {"patient": "P1", "timepoint": "S2", "response": "Progressor"},
    "sample-P2-S1": {"patient": "P2", "timepoint": "S1", "response": "Responder"},
    "sample-P2-S2": {"patient": "P2", "timepoint": "S2", "response": "Responder"},
    "sample-P3-S1": {"patient": "P3", "timepoint": "S1", "response": "Responder"},
    "sample-P3-S2": {"patient": "P3", "timepoint": "S2", "response": "Responder"},
    "sample-P4-S1": {"patient": "P4", "timepoint": "S1", "response": "Progressor"},
    "sample-P4-S2": {"patient": "P4", "timepoint": "S2", "response": "Progressor"},
    "sample-P5-S1": {"patient": "P5", "timepoint": "S1", "response": "Responder"},
    "sample-P5-S2": {"patient": "P5", "timepoint": "S2", "response": "Responder"},
    "sample-P6-S1": {"patient": "P6", "timepoint": "S1", "response": "Responder"},
    "sample-P6-S2": {"patient": "P6", "timepoint": "S2", "response": "Responder"},
}
...
def load_module3_outputs(sample_name: str) -> tuple:
    ...
    # Load cell type proportions (cuOPT QP results in per-sample subdirectory)
    prop_file = MODULE3_OUTPUT / sample_name / f"{sample_name}_cell_prop_finetuned_results.csv"
    if not prop_file.exists():
        prop_file = MODULE3_OUTPUT / sample_name / f"{sample_name}_cell_prop_global_results.csv"
...
def main():
    parser = argparse.ArgumentParser(description="Run Module 4 program discovery on patient samples")
    parser.add_argument(
        "--sample",
        type=str,
        required=True,
