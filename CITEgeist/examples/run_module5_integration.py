...
from CITEgeist.model.programs.cross_sample_integration import (
    IntegrationResult,
    integrate_samples,
...
# CITEgeist public data — set these to point at your Module 4/4b outputs
MODULE4_OUTPUT = Path("/path/to/output/module4_discovery")
MODULE4B_OUTPUT = Path("/path/to/output/module4b_7type")
...
def load_module4b_csv(sample_name: str) -> Optional[BivariateProgramResult]:
    ...
    for _, row in df.iterrows():
        pair = ProgramPairRelationship(
            ...
            top_genes_overlap=row.get("top_genes_overlap", "").split(",")
            if pd.notna(row.get("top_genes_overlap", ""))
            else [],
...
def main():
    parser = argparse.ArgumentParser(description="Run Module 5 cross-sample integration")
    parser.add_argument(
        "--output-dir",
        type=str,
