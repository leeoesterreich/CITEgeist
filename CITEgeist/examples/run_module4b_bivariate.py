...
def load_spatial_adata(sample_name: str) -> sc.AnnData:
    """
    Load spatial AnnData for a sample, subsetted to the same spots used by M4.

    The H matrix from M4 was built on spots that passed filter_gex(). To ensure
    the spot indices match, we load proportions CSV and subset adata to its index.

    Args:
        sample_name: Sample identifier

    Returns:
        AnnData with spatial coordinates, subsetted to M4-consistent spots
    """
    ...
    if "spatial" not in adata.obsm:
        pos_path = sample_path / "spatial" / "tissue_positions.csv"
        if pos_path.exists():
            pos_df = pd.read_csv(pos_path, index_col=0)
            common = adata.obs_names.intersection(pos_df.index)
            coords = pos_df.loc[common, ["pxl_col_in_fullres", "pxl_row_in_fullres"]].reindex(adata.obs_names).values
            adata.obsm["spatial"] = coords

    # Filter NaN coordinates (surgical samples have NaN spatial coords)
    if "spatial" in adata.obsm:
        coords = adata.obsm["spatial"]
        valid_mask = np.all(np.isfinite(coords), axis=1)
        if (~valid_mask).sum() > 0:
            logger.info(f"  Filtering {(~valid_mask).sum()} spots with NaN coords")
            adata = adata[valid_mask].copy()

    # Subset to spots that M4 used (match H matrix dimensions)
    ...
    if prop_file.exists():
        prop_df = pd.read_csv(prop_file, index_col=0)
        common = adata.obs_names.intersection(prop_df.index)
        if len(common) < adata.n_obs:
            logger.info(f"  Subsetting adata from {adata.n_obs} to {len(common)} spots (matching M4 proportions)")
...
def run_module4b(
    ...
    summary = {
        ...
        "relationship_summary": {
            ...
            "other": sum(
                1 for p in result.significant_pairs if p.relationship_type not in ["co-localized", "exclusive"]
            ),
        },
    }
...
def main():
    parser = argparse.ArgumentParser(description="Run Module 4b bivariate program analysis")
    parser.add_argument(
        "--sample",
        type=str,
        required=True,
    ...
    if result.significant_pairs:
        ...
        for pair in sorted_pairs[:5]:
            print(
                f"  {pair.anchor1}_{pair.program1_id} <-> {pair.anchor2}_{pair.program2_id}: "
