...
Usage:
    ...
    python run_module12_discovery.py --sample sample-P1-S1 \
...
from CITEgeist.model import analyze_marker_colocalization, identify_interesting_markers, select_profiles
...
def run_module12(
    ...
    selection_result = select_profiles(
        ...
        _interesting_markers=interesting,
    ...
    results["module2c"] = {
        ...
        "marginal_gains": [float(g) for g in selection_result.marginal_gains]
        if hasattr(selection_result, "marginal_gains")
        else [],
...
def main():
    parser = argparse.ArgumentParser(description="Run Module 1-2 discovery on patient samples")
    parser.add_argument(
        "--sample",
        type=str,
        required=True,
