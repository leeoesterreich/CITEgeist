...
def match_profiles_to_ground_truth(
    ...
    """
    Match discovered profiles to ground truth by exact marker composition.

    A match requires EXACT marker set equality - the discovered profile must
    contain exactly the same markers as the ground truth profile.
    ...
    # For each discovered profile, find exact marker match in ground truth
    for disc_name, disc_profile in discovered_profiles.items():
        ...
        for gt_name, gt_markers in gt_marker_sets.items():
            if disc_markers == gt_markers:
                matched_profiles[disc_name] = gt_name
                matched_gt_types.add(gt_name)
                found_match = True
                logger.info(f"Matched profile '{disc_name}' -> '{gt_name}'")
                break
    ...
    n_matched = len(matched_profiles)

    profile_recovery_rate = n_matched / n_gt if n_gt > 0 else 0.0
