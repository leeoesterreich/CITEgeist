...
def build_detection_marker_groups(
    active_mask: np.ndarray,
    type_names: List[str],
) -> Dict[str, List[int]]:
    """Build marker groups for GMM detection using curated strategy.

    Rule: exclusive markers for most types, plus pairwise-shared markers
    for types that have >= 2 exclusive markers (adds discriminative power
    without contaminating types that use the shared marker exclusively).

    Validated via 4-way sweep: curated r=0.561 vs exclusive-only 0.502,
    all-active 0.524, exclusive+pairwise 0.535.

    Args:
        active_mask: (T, M) bool, True where type-marker pair is active.
        type_names: (T,) list of cell type names.
    ...
    for t in range(T):
        ...
        if len(exclusive) >= 2 and pairwise:
            ...
            marker_groups[type_names[t]] = exclusive
        else:
            marker_groups[type_names[t]] = active_markers
