"""QP-based cell proportion solver for CITEgeist.

Elastic-net regularized EM optimization with per-marker beta scaling.
Uses cuOPT (cuopt.linear_programming.problem.Problem) for the per-spot
QP E-step when available; the outer EM loop and M-step are pure NumPy.
"""
...
try:
    ...
    from ..gex.gex_modules import compute_kl_penalty_coefficients, compute_softmax_target  # noqa: F401
    from ..utils import get_neighbors_with_fixed_radius  # noqa: F401

try:
    from cuopt.linear_programming.problem import Problem
except Exception:  # pragma: no cover - cuOPT needs CUDA; fails on CPU nodes (rmm CUDARuntimeError, not ImportError)
    Problem = None
...
_cell_pass2_worker_data: Optional[Dict[str, Any]] = None
