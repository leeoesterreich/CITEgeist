...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Benchmark CARD predictions against ground truth."
    )
    parser.add_argument(
        "--card_results_dir",
        type=str,
...
def main():
    ...
    card_prediction_files = sorted(
        [
            os.path.join(card_results_dir, f)
            for f in os.listdir(card_results_dir)
    ...
    ground_truth_files = sorted(
        [
            ...
            if f.endswith("_prop.csv")
        ]
    )

    # Sanity check
    if len(card_prediction_files) != len(ground_truth_files):
        print(f"Warning: Found {len(card_prediction_files)} prediction files "
    ...
    for pred_file in card_prediction_files:
        ...
        output_file = os.path.join(
            output_dir, f"{method_name}_benchmarking_metrics_rep_{rep_num}.csv"
        )
