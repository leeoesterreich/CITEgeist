...
echo -e "[${YELLOW}${START_TIME}${RESET}] Job started for replicate $SLURM_ARRAY_TASK_ID" | tee -a slurm_log/tangram_runtime.log
...
python Tangram.py \
    --spatial_path "/ix1/alee/LO_LAB/Personal/Brent_Schlegel/Projects/Wu_Visium/Simulations/scCube_12k/replicates/mixed/h5ad_objects/${REPLICATE_NAME}" \
    --output_dir "/ix1/alee/LO_LAB/Personal/Brent_Schlegel/Projects/Wu_Visium/Simulations/scCube_12k/Tangram/mixed/tangram_map_${SLURM_ARRAY_TASK_ID}" \
    --wu_reference_path "/ix1/alee/LO_LAB/General/Public_Data/BC-Datasets/Wu_2021/Wu_etal_2021_BRCA_scRNASeq" \
    --device "cuda:0"
...
echo -e "[${YELLOW}${END_TIME}${RESET}] Job completed for replicate $SLURM_ARRAY_TASK_ID" | tee -a slurm_log/tangram_runtime.log
echo -e "[${GREEN}TANGRAM_TOTAL_RUNTIME${RESET}]: Replicate $SLURM_ARRAY_TASK_ID took $RUNTIME seconds ($RUNTIME_MINUTES minutes)" | tee -a slurm_log/tangram_runtime.log
