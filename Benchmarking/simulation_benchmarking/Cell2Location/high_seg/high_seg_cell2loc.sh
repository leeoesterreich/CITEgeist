#!/bin/bash
#SBATCH --job-name=hs_cell2location  # Job name
...
echo -e "[${YELLOW}${START_TIME}${RESET}] Job started for replicate $SLURM_ARRAY_TASK_ID" | tee -a logs/hs_cell2location_runtime.log
...
source /ix1/alee/LO_LAB/Personal/Brent_Schlegel/miniconda3/bin/activate /ix1/alee/LO_LAB/Personal/Brent_Schlegel/envs/CITEgeist
if [ $? -ne 0 ]; then
...
cd /ix1/alee/LO_LAB/Personal/Brent_Schlegel/Projects/Wu_Visium/Simulations/scCube_12k/cell2location/high_seg
if [ $? -ne 0 ]; then
...
# Run Python script
echo -e "${BLUE}Running Python script: high_seg_cell2loc.py${RESET}"
python high_seg_cell2loc.py "${REPLICATE_NAME}"
...
echo -e "[${YELLOW}${END_TIME}${RESET}] Job completed for replicate $SLURM_ARRAY_TASK_ID" | tee -a logs/hs_cell2location_runtime.log
echo -e "[${GREEN}HS_CELL2LOCATION_TOTAL_RUNTIME${RESET}]: Replicate $SLURM_ARRAY_TASK_ID took $RUNTIME seconds ($RUNTIME_MINUTES minutes)" | tee -a logs/hs_cell2location_runtime.log
