...
DATA_PROTEIN_GT = REPO_ROOT / "Benchmarking" / "xenium_pseudovisium" / "data_protein_gt"


def main():
    parser = argparse.ArgumentParser(
        description="Generate GEX ground truth from protein-gated cell types"
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    cell_types = pd.read_csv(
        DATA_PROTEIN_GT / "cell_type_assignments.csv", index_col=0
    )["cell_type"]
    cell_to_spot = pd.read_csv(
        DATA_PROTEIN_GT / "cell_to_spot_mapping.csv", index_col=0
    )
