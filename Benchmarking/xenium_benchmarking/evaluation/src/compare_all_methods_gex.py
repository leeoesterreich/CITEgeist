...
and Tangram. RCTD and Seurat only output proportions, so they are excluded.
...
def load_citegeist_gex(region_id: int) -> Dict[str, pd.DataFrame]:
    ...
    base_layers = BASE_DIR / "CITEgeist" / "output_protein_gt" / f"{sample_name}_pass1" / "layers"
...
def main():
    ...
    for method_name in ["CITEgeist", "Cell2Location", "Tangram"]:
    ...
    sorted_methods = [m for m in ["CITEgeist", "Cell2Location", "Tangram"] if all_results.get(m)]
