#!/bin/bash
#SBATCH --job-name=xenium_achievable_7           # Job name
...
#SBATCH --array=0-4                              # 5 regions (granular GT data)
#SBATCH --mail-type=ALL
#SBATCH --mail-user=alc376@pitt.edu

# ============================================================================
# Xenium Benchmarking - CITEgeist with ACHIEVABLE-7 Cell-Type Profiles
# ============================================================================
# This script runs CITEgeist on pseudo-Visium spots using 7 cell types that
# can be realistically distinguished by the 27-antibody panel.
#
# Key difference from 8-type: Myofibroblasts + Stromal → Fibroblasts
# Both express high Vimentin (~99% of cells), alphaSMA overlaps (108 vs 20 mean)
#
# The 10 granular GT types are collapsed to 7 achievable types during eval:
...
#
# Achievable cell types (7):
# 1. B cells         - CD20, CD45RA
# 2. CD4+ T cells    - CD3E, CD4, CD45RO
# 3. CD8+ T cells    - CD3E, CD8A, GranzymeB
# 4. Macrophages     - CD68, CD163, CD16
# 5. Endothelial     - CD31
# 6. Epithelial      - PanCK (E-Cadherin removed - does not co-localize)
# 7. Fibroblasts     - alphaSMA, Vimentin
# ============================================================================
...
OUTPUT_DIR="${CITEGEIST_DIR}/output_achievable_7"
...
echo -e "[${YELLOW}${START_TIME}${RESET}] Job started for region $SLURM_ARRAY_TASK_ID" | tee -a "${SLURM_LOG_DIR}/xenium_achievable_7_runtime.log"
...
# ============================================================================
# Run CITEgeist with ACHIEVABLE-7 cell-type profiles
# ============================================================================

REGION_ID=$SLURM_ARRAY_TASK_ID
echo -e "${YELLOW}Processing region ${REGION_ID} with ACHIEVABLE-7 cell-type profiles${RESET}"
...
# Run CITEgeist benchmark with ACHIEVABLE-7 cell profiles (7 cell types)
...
echo -e "[${YELLOW}${END_TIME}${RESET}] Job completed for region ${REGION_ID}" | tee -a "${SLURM_LOG_DIR}/xenium_achievable_7_runtime.log"
echo -e "[${GREEN}XENIUM_ACHIEVABLE_7_RUNTIME${RESET}]: Region ${REGION_ID} took ${RUNTIME} seconds (${RUNTIME_MINUTES} minutes)" | tee -a "${SLURM_LOG_DIR}/xenium_achievable_7_runtime.log"

echo -e "${GREEN}======================================${RESET}"
echo -e "${GREEN}Region ${REGION_ID} ACHIEVABLE-7 benchmark complete${RESET}"
echo -e "${GREEN}======================================${RESET}"
