...
def test_create_adata_attaches_validated_functional_annotations():
    cell_gex = pd.DataFrame(
        [[1.0, 2.0]],
        index=["c1"],
        columns=["G1", "G2"],
    )
    assignments = {"c1": "CD8_T_Cells"}
    functional = pd.DataFrame(
        {
            "func_PDCD1_CD8_T_Cells_call": [1],
            "func_PDCD1_CD8_T_Cells_score": [0.95],
        },
        index=["c1"],
    )
    ...
    adata = create_single_cell_adata(
        ...
        assignments=assignments,
        sample_name="sample1",
        functional_annotations=functional,
        functional_metadata={"validated_pairs": ["CD8_T_Cells__PDCD1"]},
    )

    assert "func_PDCD1_CD8_T_Cells_call" in adata.obs.columns
    assert "func_PDCD1_CD8_T_Cells_score" in adata.obs.columns
    assert adata.obs.loc["c1", "func_PDCD1_CD8_T_Cells_call"] == 1
    assert adata.uns["functional_annotation_meta"]["validated_pairs"] == ["CD8_T_Cells__PDCD1"]
