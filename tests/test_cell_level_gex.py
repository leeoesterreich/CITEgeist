...
def test_allocate_gex_type_reference_preserves_spot_totals():
    ...
    from CITEgeist.model.gex.cell_level_gex import allocate_gex_type_reference

    spot_gex = pd.DataFrame({"gene_A": [10.0, 20.0], "gene_B": [5.0, 15.0]}, index=["spot_0", "spot_1"])
    proportions = pd.DataFrame({"TypeX": [0.6, 0.3], "TypeY": [0.4, 0.7]}, index=["spot_0", "spot_1"])
...
class TestDistributeGexDeprecation:
    def test_deprecation_warning(self):
        from CITEgeist.model.gex.cell_level_gex import distribute_gex_to_cells

        deconvolved_gex = pd.DataFrame(
            {"gene1": [10.0, 20.0]},
            index=["spot1:::TypeA", "spot1:::TypeB"],
        )
        assignments = {1: "TypeA", 2: "TypeB"}
        nucleus_spot_map = pd.DataFrame(
            {
                "nucleus_id": [1, 2],
                "spot_id": ["spot1", "spot1"],
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            distribute_gex_to_cells(deconvolved_gex, assignments, nucleus_spot_map)
            assert len(w) == 1
            assert issubclass(w[0].category, DeprecationWarning)
            assert "project_sace_to_cells" in str(w[0].message)
